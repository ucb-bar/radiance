...
case class VirgoClusterParams(
  clusterId: Int,
  clockSinkParams: ClockSinkParameters = ClockSinkParameters()
) extends InstantiableClusterParams[VirgoCluster] {
  val baseName = "virgo_cluster"
  ...
  def instantiate(crossing: HierarchicalElementCrossingParamsLike, lookup: LookupByClusterIdImpl)
                 (implicit p: Parameters): VirgoCluster = {
    new VirgoCluster(this, crossing.crossingType, lookup)
  }
}

class VirgoCluster (
  thisClusterParams: VirgoClusterParams,
...
class VirgoClusterModuleImp(outer: VirgoCluster) extends ClusterModuleImp(outer) {
  println(s"======= VirgoCluster: clbus inward edges = ${outer.clbus.inwardNode.inward.inputs.length}")
  println(s"======= VirgoCluster: clbus name = ${outer.clbus.busName}")
